...
import org.tensorflow.demo.DetectionLabel;
 ...
 * objects.
 */
public class BlackJack extends CameraActivity implements OnImageAvailableListener {
          ...
          public void run() {
            ...
            for (final Classifier.Recognition result : results) {
              ...
              if (location != null && result.getConfidence() >= minimumConfidence) {
                ...
                final DetectionLabel mesObjets = new DetectionLabel();
                runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                  for (int j = 0; j < mesObjets.getLabelsSearched().size(); j++) {
                                    if (mesObjets.getLabelsSearched().get(j).equals(result.getTitle())) {
                                      LOGGER.i("Label à trouver numero %d detecte (il s'agit du label %s)", j + 1, result.getTitle());
                                      mesObjets.getLabelsSearched().remove(j);
                                     /*@Override
                                     public void onCreate(Bundle savedInstanceState){*/
                                      BlackJack.super.onCreate(null);


                                      if (mesObjets.getLabelsSearched().size() == 3) {
                                        setContentView(R.layout.layout_3_objects);
                                        Intent intentMyAccount = new Intent(getApplicationContext(), BlackJack.class);
                                        startActivity(intentMyAccount);
                                      } else if (mesObjets.getLabelsSearched().size() == 2) {
                                        setContentView(R.layout.layout_2_objects);
                                        Intent intentMyAccount = new Intent(getApplicationContext(), BlackJack.class);
                                        startActivity(intentMyAccount);
                                      } else if (mesObjets.getLabelsSearched().size() == 1) {
